...
- امکان اجرای همه سرویس‌های فعال توسط مدیر از بخش «ثبت استعلام»، بدون نیاز به تخصیص سرویس یا کسر از کیف پول مدیر؛ درخواست‌ها با نام مدیر ثبت می‌شوند
...
- ورودی تصویر برای فایل‌های JPG، PNG و WebP؛ تبدیل خودکار به رشته Base64 خام در payload سرویس مقصد
...
برای ورودی تصویر، نوع فیلد را «تصویر (Base64 خودکار)» انتخاب کنید. کاربر فایل را بارگذاری می‌کند و سامانه پس از اعتبارسنجی، رشته Base64 خام (بدون پیشوند `data:image/...`) را با کلید همان فیلد به سرویس مقصد می‌فرستد. سقف پیش‌فرض هر فایل ۲۰۴۸ کیلوبایت و مجموع تصاویر هر درخواست ۴۰۹۶ کیلوبایت است؛ این مقادیر با `REMOTE_SERVICE_MAX_IMAGE_KILOBYTES` و `REMOTE_SERVICE_MAX_TOTAL_IMAGE_KILOBYTES` قابل تنظیم‌اند. تصویر در سوابق به‌صورت رمزنگاری‌شده ذخیره می‌شود، در جزئیات نمایش داده نمی‌شود و «اجرای مجدد با همین اطلاعات» از تصویر قبلی استفاده می‌کند. در «ویرایش و اجرای مجدد» باید تصویر تازه بارگذاری شود.
