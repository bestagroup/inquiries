...
<form method="POST" action="{{ $service->exists ? route('admin.services.update',$service) : route('admin.services.store') }}">
    ...
    <div class="card mb-3"><div class="card-header"><strong>تعریف سرویس</strong></div><div class="card-body"><div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">دسته‌بندی</label><input class="form-control" name="category" value="{{ old('category',$service->category) }}" placeholder="احراز هویت"></div>
        <div class="col-md-4"><label class="form-label">آیکن Bootstrap</label><input class="form-control" dir="ltr" name="icon" value="{{ old('icon',$service->icon) }}" placeholder="bi-person-check"></div>
